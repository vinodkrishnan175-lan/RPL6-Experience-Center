...
def safe_read_excel(uploaded_file, sheet=None, header=0):
    """Read a single sheet as a DataFrame.
    If sheet is None, read the first sheet (sheet_name=0). This avoids pandas returning a dict when sheet_name=None.
    """
    try:
...
# Safety: if something still returns a dict of sheets, take the first sheet
...
# Sidebar diagnostics (helps verify all 48 are loaded)
with st.sidebar:
    st.caption(f"Loaded players: {metrics['Player'].nunique()} | Perfect (24/24): {(metrics['Attended_out_of_24']==TOTAL_VALID_DROPS).sum()}")
...
figm.update_traces(marker=dict(size=12, opacity=0.9))
