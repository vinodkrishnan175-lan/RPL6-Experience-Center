...
def load_data(uploaded_file: "st.runtime.uploaded_file_manager.UploadedFile" = None) -> pd.DataFrame:
    """Load the canonical Excel (sheet: Player Metrics)."""
...
def pop_help(label: str, body: str):
    # A tiny "?" badge with an attached popover (fast, not clipped)
    with st.popover("?", help=label):
        st.write(body)

def psych_read(pp_used_total: int, b1: str, b2: str) -> str:
    """One-sentence psychological read. Positive, non-judgy."""
    if pp_used_total == 0:
...
# -----------------------------
# UI: Header + Data Loader
# -----------------------------
st.title("RPL 6 — Behavioral Dashboard")
st.caption("Behavioral mirror + tournament context. Built for curiosity (not clutter).")
...
# Fill missing columns if needed (defensive)
...
# Prefer provided archetype definition, else map
df["Archetype Definition"] = df["Archetype Definition"].fillna(df["Archetype"].map(ARCH_DEF))

# Coerce numerics
num_cols = ["AttendancePct","Attended","TotalDrops","H%","M%","L%","Volatility%","AvgRisk","PP_used_total","PP_Q1","PP_Q2"]
for c in num_cols:
    df[c] = pd.to_numeric(df[c], errors="coerce")

# -----------------------------
# Global Player Selector (subtle)
# -----------------------------
...
with c1:
    ...
    total_drops = int(row["TotalDrops"]) if pd.notna(row["TotalDrops"]) else 0
    if total_drops <= 0:
        total_drops = int(df["TotalDrops"].dropna().iloc[0]) if df["TotalDrops"].notna().any() else 0
...
st.caption("Fast, shareable insights. (Names shown only for players with ≥30% attendance; Sai/Arvind excluded.)")

story_df = df.copy()
story_df["AttendancePct"] = pd.to_numeric(story_df["AttendancePct"], errors="coerce")
story_df = story_df[story_df["AttendancePct"].fillna(0) >= 30]
story_df = story_df[~story_df["Player_Clean"].str.lower().isin(["sai","arvind"])]

total_responses = int(df["Attended"].fillna(0).sum())
consensus_responses = int(np.round((df["Attended"].fillna(0) * df["H%"].fillna(0) / 100).sum()))

perfect = df[(df["Attended"] == df["TotalDrops"]) & df["Player_Clean"].notna()].copy().sort_values("Player_Clean")
both_pp = story_df[story_df["PP_used_total"].fillna(0) >= 2].copy().sort_values("PP_Q2")
contrarian = story_df.sort_values("L%", ascending=False).head(5)
anchors = story_df.sort_values("Volatility%").head(5)
no_pp = story_df[story_df["PP_used_total"].fillna(0) == 0].sort_values("Player_Clean")

tabs = st.tabs([
    "Meta", "Contrarian League", "Anchors", "Conviction Used Early", "Conviction Still Loaded", "Perfect Attendance Club"
])
...
# -----------------------------
# SECTION: Risk vs Attendance Map
# -----------------------------
...
points_plotted = int(plot_df["Player_Clean"].nunique())
st.caption(f"Points plotted: **{points_plotted}** (should be 48)")
st.plotly_chart(fig, use_container_width=True)

st.divider()

# -----------------------------
# SECTION: Power Play Status (3 boxes)
# -----------------------------
...
with b1:
    st.markdown("### Exhausted both Power Plays")
    d = df[df["PP_used_total"].fillna(0) >= 2].copy().sort_values("PP_Q2")
    if len(d):
        d["Exhausted by"] = d["PP_Q2"].apply(lambda x: f"Q{int(x)}" if pd.notna(x) else "—")
        st.dataframe(d[["Player_Clean","Exhausted by"]].rename(columns={"Player_Clean":"Player"}), hide_index=True, use_container_width=True)
    else:
        st.write("—")

with b2:
    st.markdown("### Used Power Play only once")
    d = df[df["PP_used_total"].fillna(0) == 1].copy().sort_values("PP_Q1")
    if len(d):
        d["Used on"] = d["PP_Q1"].apply(lambda x: f"Q{int(x)}" if pd.notna(x) else "—")
        st.dataframe(d[["Player_Clean","Used on"]].rename(columns={"Player_Clean":"Player"}), hide_index=True, use_container_width=True)
    else:
        st.write("—")

with b3:
    st.markdown("### No Power Play used")
    d = df[df["PP_used_total"].fillna(0) == 0].copy().sort_values("Player_Clean")
    st.write(", ".join(d["Player_Clean"].tolist()) if len(d) else "—")

if show_debug:
    st.divider()
    st.subheader("Sanity Check (temporary)")
    st.write(f"- Total players: **{int(df['Player_Clean'].nunique())}** (should be 48)")
    st.write(f"- Perfect attendance players: **{int((df['Attended'] == df['TotalDrops']).sum())}**")
